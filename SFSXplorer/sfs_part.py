...
def process_csv_chunk(self,chunk):
    ...
    for line in chunk:
        ...
        string_VDW=""
        n_exp_range = np.arange(self.pot_VDW_n_min, self.pot_VDW_n_max+1)
        m_exp_range = np.arange(self.pot_VDW_m_min, self.pot_VDW_m_max+1)
        ...
        n_exp_range = np.arange(self.pot_HB_n_min, self.pot_HB_n_max+1)
        m_exp_range = np.arange(self.pot_HB_m_min, self.pot_HB_m_max+1)
        for n_exp in n_exp_range:
            for m_exp in m_exp_range:
        ...
        for m in self.m_array_desol:
            for n in self.n_array_desol:
                for sigma in self.s_array_desol:
                    # Calculate potential
        ...
        for count,ele in enumerate(line):
            if count != len(line)-1:
                ...
                data_in += line[count]
    lines_out.append(data_in + line_VDW_HB + v_Elec_logistic + v_Elec_tanh + v_Elec_logistic_tanh + v_Desol + "\n")
    return lines_out
...
# Define read_input() method
    ...
    def write_energy1(self):
         # Split `self.csv0` into chunks for parallel processing.
        ...
        def get_headers(n_min, n_max, m_min, m_max, prefix):
            ...
            headers = [f"{prefix}_{n_exp}_{m_exp}" for n_exp in n_exp_range for m_exp in m_exp_range if n_exp != m_exp]
            return headers
        self.a_array = np.linspace(self.A_i, self.A_f, self.n_A)
        self.e0_array = np.linspace(self.epsilon0_i, self.epsilon0_f, self.n_epsilon0)
        self.k_array = np.linspace(self.k_i, self.k_f, self.n_k)
        self.l_array = np.linspace(self.lambda_i, self.lambda_f, self.n_lambda)
        self.m_array_desol = np.linspace(self.m_desol_i, self.m_desol_f, self.n_m_desol)
        self.n_array_desol = np.linspace(self.n_desol_i, self.n_desol_f, self.n_n_desol)
        self.s_array_desol = np.linspace(self.sigma_desol_i, self.sigma_desol_f, self.n_sigma_desol)
        ...
        elec_headers = [f"v_Elec_Log_{a}_{e0}_{k}_{l},v_Elec_Tanh_{a}_{e0}_{k}_{l},v_Elec_Log_Tanh_{a}_{e0}_{k}_{l}"           for a in self.a_array                for e0 in self.e0_array                for k in self.k_array                for l in self.l_array]
        desol_headers = [f"v_Desol_{m}_{n}_{sigma}" for m in self.m_array_desol         for n in self.n_array_desol                 for sigma in self.s_array_desol]
        ...
        self.fo1.write(header_out + "\n")
        num_processes = 4
        csv_chunks = [[] for _ in range(num_processes)]
        for i, line in enumerate(self.csv0):
        ...
        with Pool(num_processes) as pool:
            results = pool.map(process_csv_chunk(self), csv_chunks)
        # Write the results to the output file.
        for lines_out in results:
